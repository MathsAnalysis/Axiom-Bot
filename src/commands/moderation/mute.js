...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('mute')
    .setDescription('Mute a user (timeout-based)')
  ...
  async execute(interaction) {
    ...
    if (!hasCommandPermission(interaction.member, 'mute')) {
      return interaction.reply({
        embeds: [errorEmbed(i18n.t('general.no_permission'))],
        ephemeral: true,
      });
    ...
    // DM the user before muting (if configured)
    if (config.get('moderation.dm_on_action')) {
      try {
        const dmEmbed = modEmbed(
          'Mute',
          i18n.t('moderation.mute_dm', {
    ...
    try {
      ...
      console.error('[Mute] Failed to mute user:', error);
      return interaction.reply({
        embeds: [errorEmbed(i18n.t('general.error'))],
        ephemeral: true,
      });
    ...
    const logEmbed = modEmbed(
      'Mute',
      i18n.t('moderation.mute_success', {
        user: targetUser.tag,
        duration: durationFormatted,
        reason,
